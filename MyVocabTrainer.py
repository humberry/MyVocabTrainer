...
class VocabTrainer(object):
  ...
  def bt_training_Okay(self, sender):
    ...
    self.make_label('label1', word, (self.view.width/2)-(size[0]/2), int(self.fontsize), size[0], int(self.fontsize), self.font, int(self.fontsize), color=self.colorFL) #(self.view.height/2)-size[1]
    button = ui.Button()
  ...
  def show_second_page(self):
    ...
    self.curs.close()
    self.conn.close()
    if self.direction == 3:  # swap language if mixed is choosed
      if self.next_language == 1:
        self.next_language = 2
      else:
        self.next_language = 1
    ...
    self.make_label('label1', word, (self.view.width/2)-(size[0]/2), height, size[0], int(self.fontsize), self.font, int(self.fontsize), color=self.colorSL) #(self.view.height/2)-size[1]
    self.make_button('button1', 'Learned', 70, height + 100, 300, 150, action=self.bt_learned)
    self.make_button('button2', 'Not Learned', 380, height + 100, 300, 150, action=self.bt_not_learned)
